...
app.put("/verify",varifyToken, async (req, res) => {
    try {
        ...
        let mailOption = {
            ...
            to: user.email,
            subject: "Payment Done ! : Team eMart",
        ...
        if (schema.validate(req.body.password)) {
            bcrypt.hash(req.body.password, 12, async function (err, hash) {
                ...
                let mailOption = {
                    from: process.env.MAILSENDER,
                    to: data.email,
                    subject: "Your Account is Created ! : Team eMart",
                    text: `Thanks to Create an Account With US! Team eMart`
                }
                transporter.sendMail(mailOption, (error, data) => {
                    if (error)
                        console.log(error);
...
app.post("/newslatter", async (req, res) => {
    try {
        ...
        let mailOption = {
            from: process.env.MAILSENDER,
            to: data.email,
            subject: "Email Is Registered ! : Team eMart",
            text: `Thanks to Subscribe our Newsletter Service\nNow we will send  you e-mails regrading about our Latest Products and new Offers\nTeam eMart`
        }
        transporter.sendMail(mailOption, (error, data) => {
            if (error)
                console.log(error);
        })
        res.send({ result: "Done", message: "Thanks to Subscribe our Newsletter Service\nNow We will Send e-mails regarding Latest Products and Offers." })
    }
    catch (error) {
        // console.log(error)
        if (error.keyValue)
...
app.post("/contact", async (req, res) => {
    try {
        ...
        let mailOption = {
            from: process.env.MAILSENDER,
            to: data.email,
            subject: "Your Query Received ! : Team eMart",
...
app.post("/checkout", varifyToken, async (req, res) => {
    try {
        ...
        let mailOption = {
            from: process.env.MAILSENDER,
            to: user.email,
            subject: "Your Order is Placed ! : Team eMart",
            text: `Thanks to Place order\nNow You Can Track Your Order is Profile Section!\nTeam eMart`
        }
        transporter.sendMail(mailOption, (error, data) => {
            if (error)
                console.log(error);
    ...
    try {
        ...
        if (data) {
            ...
            let mailOption = {
                from: process.env.MAILSENDER,
                to: data.email,
                subject: "OTP for Password Reset ! : Team eMart",
